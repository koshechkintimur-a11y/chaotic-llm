"""exp42_vector_gates.py — TEST H3: do d-dimensional gates unblock d=512 scaling?

Hypothesis (exp41b): scalar gates (`g` shared across all d dims) are the
architectural ceiling — mixing capacity doesn't scale with width.

Fix: vector gates — `gates_l = nn.Parameter(torch.zeros(bl, d))`, each
dimension gets its own mixing coefficient. Cost: +bl*d params (negligible),
computation identical (element-wise broadcast).

Same protocol as exp41b (lr 5e-4, warmup, cosine 16K, clipping, diagnostics).
If mixer-only PPL breaks below 16.7 → H3 confirmed and architecture scales.
"""
import os
import sys
import json
...
OUT = os.path.join(HERE, "exp42_vector_gates")
...
class ChaoticBlock(nn.Module):
    def __init__(self, W, Wl, d, bl, bg, vector_gates=True):
        super().__init__()
        self.W, self.Wl, self.Nw = W, Wl, W // Wl
        self.d = d
        self.vector_gates = vector_gates
        if vector_gates:
            self.gates_l = nn.Parameter(torch.zeros(bl, d))  # (bl, d)
    ...
    def _chaotic(self, h, sigmas, gates):
        ...
        for t in range(1, len(gates) + 1):
            ...
            g = torch.sigmoid(gates[t - 1])   # scalar or (d,)
            even = h[:, 0::2, :]
            odd = h[:, 1::2, :]
...
class ChaoticBase(nn.Module):
    def __init__(self, V, W, Wl, d, bl, bg):
        ...
        self.block = ChaoticBlock(W, Wl, d, bl, bg, vector_gates=True)
...
print(f"params: {n_params:,}  device: {DEVICE}  (vector gates)")
...
n = len(train_ids) - W - 1
t0 = time.time()
print("training (16K, lr 5e-4, warmup+cosine, VECTOR gates)...")
...
torch.save(model.state_dict(), os.path.join(OUT, "mixer_d512_vecgates.pt"))
...
print(f"  [scalar gates d=512: 97.4 | d=256: 16.73 | d=64: 32.78]")
