...
- Все числа — на **публичном** корпусе (TheAlgorithms/Python, MIT), воспроизводимы вне этой машины (см. TODO #5 и `SCIENCE_AUDIT.md`).
...
- [x] Честный финальный прогон (v1, `final_benchmark.py`, 3 сида) + строгий воспроизводимый v2 (`final_benchmark_v2.py`, 5 сидов, публичный корпус, 95% CI): STS-Prog PPL 24.5 vs TF-matched 46.6; таблицы влиты через `render_docs_v2.py` (см. TODO #5 и `SCIENCE_AUDIT.md`)
...
**Статус: открыто** (ночной прогон на Stack; в v2 не трогали).
...
**5. Воспроизводимость, параметр-матч и статистика малого прогона — ✅ ЗАКРЫТО.**
